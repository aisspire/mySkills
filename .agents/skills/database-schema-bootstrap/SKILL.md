...
- 显式指定 profile 文件时使用 `--config <path>`；
- 配置缺失：只询问缺失的非密钥字段
...
`profile` 是项目配置文件的概念；`inspect_schema.py` 的 CLI 参数名是 `--config`。
...
```powershell
python '.agents\skills\database-schema-bootstrap\scripts\inspect_schema.py' list-tables --config '.agents\database-schema-bootstrap\profiles\default.json'
```
...
```powershell
python '.agents\skills\database-schema-bootstrap\scripts\inspect_schema.py' describe --config '.agents\database-schema-bootstrap\profiles\default.json' --table users --table orders
```

如果不确定子命令参数，先查看子命令 help：

```powershell
python '.agents\skills\database-schema-bootstrap\scripts\inspect_schema.py' describe --help
```
...
显式指定 profile 文件时必须使用 `--config <path>`，例如：

```powershell
python '.agents\skills\database-schema-bootstrap\scripts\inspect_schema.py' describe --config '.agents\database-schema-bootstrap\profiles\default.json' --table users
```

不要使用 `--profile`；当前 CLI 不支持该参数。
